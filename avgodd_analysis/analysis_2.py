...
try:
    ...
    league_name_arr = ['德甲', '英超', '法甲', '法乙', '俄超', '比甲', '德乙', '乌超', '丹超', '英甲', '西甲', '丹甲', '捷甲', '意乙', '西乙', '波甲', '芬超', '奥乙', '奥甲',
                                ...
                                '葡超', '巴西甲', '墨超', '巴西乙', '葡甲', '阿甲', '罗甲', '克罗甲', '塞甲联', '以超', '保超', '哥甲', '欧罗巴']
    limit_odd = 0
    max_odd = 1.5
    expected_result = 3
    low_limit = 12
    ...
    for league_name in league_name_arr:
        ...
        # 显示利润
        if total == 0:
            print('联赛名称: %s，总数：%s' % (league_name, 0))
        else:
            print('联赛名称: %s，总数：%s, 命中率：%s, 利润：%s, 利润率：%s' % (league_name, total, round(shot/total, 3), round(profit, 3), round((profit)/total, 3)))
        # 显示所画的图
        if show_pic:
            plt.show()
