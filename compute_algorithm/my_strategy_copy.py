...
class My_strategy:
    ...
    def get(self, league_name, value_ratio):
        if not league_name in self.strategy_league:
            return ''
        if league_name == '德甲':
            if is_between(value_ratio, 0.1, 0.3) or is_between(value_ratio, 0.6, 0.7):
                return 3
                ...
                return 0
        if league_name == '英超':
            if is_between(value_ratio, 0.5, 0.6) or is_between(value_ratio, 0.9, 1):
                return 3
            elif is_between(value_ratio, 0.8, 0.9):
                return 1
            elif is_between(value_ratio, 0, 0.1) or is_between(value_ratio, 1, 1.1) or is_between(value_ratio, 1.67, 2):
                return 0
        if league_name == '法甲':
            if is_between(value_ratio, 0, 0.1):
                return 3
            elif is_between(value_ratio, 0.7, 0.8) or is_between(value_ratio, 2.5, 3.33):
                return 0
        if league_name == '法乙':
            if is_between(value_ratio, 1.43, 1.67):
                ...
                return 1
        if league_name == '英冠':
            if is_between(value_ratio, 0.1, 0.3) or is_between(value_ratio, 10):
                return 3
        if league_name == '俄超':
            if is_between(value_ratio, 0, 0.1) or is_between(value_ratio, 0.5, 0.6):
                return 0
            elif is_between(value_ratio, 1, 1.1):
                return 3
            elif is_between(value_ratio, 2, 2.5):
                return 1
        if league_name == '比甲':
            if is_between(value_ratio, 2, 2.5):
                ...
                return 3
        if league_name == '德乙':
            if is_between(value_ratio, 0.6, 0.7) or is_between(value_ratio, 1.67, 2.5):
                return 3
            elif is_between(value_ratio, 0.7, 0.8):
                return 1
        if league_name == '乌超':
        ...
        if league_name == '瑞典甲':
            if is_between(value_ratio, 10):
                return 3
        if league_name == '俄甲':
            if is_between(value_ratio, 0.2, 0.3) or is_between(value_ratio, 0.7, 0.8):
                return 0
        if league_name == '苏超':
        ...
        if league_name == '墨超':
            if is_between(value_ratio, 1.67, 2):
                return 1
        if league_name == '巴西乙':
            if is_between(value_ratio, 1.25, 1.43):
                return 1
        if league_name == '葡甲':
            if is_between(value_ratio, 0.2, 0.3) or is_between(value_ratio, 1.25, 1.43):
                return 0
        if league_name == '阿甲':
            if is_between(value_ratio, 1.1, 1.25):
                return 1
