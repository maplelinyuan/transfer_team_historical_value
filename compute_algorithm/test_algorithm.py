...
try:
    ...
    for cur_index in range(len(index_arr)):
        ...
        for single_match in coll.find({'league_name': league_name}):
            ...
            # 获取主客身价
            try:
                home_value = value_coll.find_one({'$and': [{'name': english_home_name}, {"current_time": need_value_time}]})['value']
                away_value = value_coll.find_one({'$and': [{'name': english_away_name}, {"current_time": need_value_time}]})['value']
    ...
    plt.xlabel('x')
    plt.ylabel('y')
    plt.title('value ratio_for_%s' % buy_result)
